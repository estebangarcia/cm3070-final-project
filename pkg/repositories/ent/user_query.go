...
type UserQuery struct {
	...
	withJoinedOrganizations *OrganizationMembershipQuery
...
func (uq *UserQuery) QueryOrganizationInvites() *OrganizationInviteQuery {
	query := (&OrganizationInviteClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryJoinedOrganizations chains the current query on the "joined_organizations" edge.
func (uq *UserQuery) QueryJoinedOrganizations() *OrganizationMembershipQuery {
	query := (&OrganizationMembershipClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOrganizationInvites: uq.withOrganizationInvites.Clone(),
		withJoinedOrganizations: uq.withJoinedOrganizations.Clone(),
...
func (uq *UserQuery) WithOrganizationInvites(opts ...func(*OrganizationInviteQuery)) *UserQuery {
	query := (&OrganizationInviteClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOrganizationInvites = query
	return uq
}

// WithJoinedOrganizations tells the query-builder to eager-load the nodes that are connected to
// the "joined_organizations" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithJoinedOrganizations(opts ...func(*OrganizationMembershipQuery)) *UserQuery {
	query := (&OrganizationMembershipClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withJoinedOrganizations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			...
			uq.withOrganizationInvites != nil,
			uq.withJoinedOrganizations != nil,
		}
	)
	...
	if query := uq.withOrganizationInvites; query != nil {
		if err := uq.loadOrganizationInvites(ctx, query, nodes,
			func(n *User) { n.Edges.OrganizationInvites = []*OrganizationInvite{} },
			func(n *User, e *OrganizationInvite) {
				n.Edges.OrganizationInvites = append(n.Edges.OrganizationInvites, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := uq.withJoinedOrganizations; query != nil {
		if err := uq.loadJoinedOrganizations(ctx, query, nodes,
			func(n *User) { n.Edges.JoinedOrganizations = []*OrganizationMembership{} },
			func(n *User, e *OrganizationMembership) {
				n.Edges.JoinedOrganizations = append(n.Edges.JoinedOrganizations, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadOrganizationInvites(ctx context.Context, query *OrganizationInviteQuery, nodes []*User, init func(*User), assign func(*User, *OrganizationInvite)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(organizationinvite.FieldUserID)
	}
	query.Where(predicate.OrganizationInvite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OrganizationInvitesColumn), fks...))
	}))
...
func (uq *UserQuery) loadJoinedOrganizations(ctx context.Context, query *OrganizationMembershipQuery, nodes []*User, init func(*User), assign func(*User, *OrganizationMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrganizationMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.JoinedOrganizationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
