...
func (oq *OrganizationQuery) QueryRegistries() *RegistryQuery {
	query := (&RegistryClient{config: oq.config}).Query()
...
func (oq *OrganizationQuery) QueryOrgMembers() *OrganizationMembershipQuery {
	query := (&OrganizationMembershipClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithRegistries(opts ...func(*RegistryQuery)) *OrganizationQuery {
	query := (&RegistryClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withRegistries = query
	return oq
}
...
func (oq *OrganizationQuery) WithOrgMembers(opts ...func(*OrganizationMembershipQuery)) *OrganizationQuery {
	query := (&OrganizationMembershipClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrgMembers = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withRegistries; query != nil {
		if err := oq.loadRegistries(ctx, query, nodes,
	...
	if query := oq.withOrgMembers; query != nil {
		if err := oq.loadOrgMembers(ctx, query, nodes,
			func(n *Organization) { n.Edges.OrgMembers = []*OrganizationMembership{} },
			func(n *Organization, e *OrganizationMembership) { n.Edges.OrgMembers = append(n.Edges.OrgMembers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadRegistries(ctx context.Context, query *RegistryQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Registry)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_registries
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_registries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_registries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (oq *OrganizationQuery) loadOrgMembers(ctx context.Context, query *OrganizationMembershipQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrganizationMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OrgMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
