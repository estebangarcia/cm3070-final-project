...
func (rc *RepositoryCreate) AddManifestIDs(ids ...int) *RepositoryCreate {
	rc.mutation.AddManifestIDs(ids...)
	return rc
}

// AddManifests adds the "manifests" edges to the Manifest entity.
func (rc *RepositoryCreate) AddManifests(m ...*Manifest) *RepositoryCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
...
func (rc *RepositoryCreate) createSpec() (*Repository, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.ManifestsIDs(); len(nodes) > 0 {
