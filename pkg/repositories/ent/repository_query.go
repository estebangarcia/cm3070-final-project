...
func (rq *RepositoryQuery) QueryManifests() *ManifestQuery {
	query := (&ManifestClient{config: rq.config}).Query()
...
func (rq *RepositoryQuery) WithManifests(opts ...func(*ManifestQuery)) *RepositoryQuery {
	query := (&ManifestClient{config: rq.config}).Query()
...
func (rq *RepositoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Repository, error) {
	...
	if query := rq.withManifests; query != nil {
...
func (rq *RepositoryQuery) loadManifests(ctx context.Context, query *ManifestQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *Manifest)) error {
	...
	query.withFKs = true
	query.Where(predicate.Manifest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(repository.ManifestsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.repository_manifests
		if fk == nil {
			return fmt.Errorf(`foreign-key "repository_manifests" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
